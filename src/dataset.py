...
class SyntheticBanditDatasetLimittedSupply(BaseBanditDataset):
    ...
    def obtain_batch_bandit_feedback(self, ) -> BanditFeedback:
        ...
        fixed_increace = np.sort(self.random_.uniform(low=0.0, high=fixed_q_x_a_base.max(), size=(self.n_users, self.n_actions)), axis=1)
        fixed_q_x_a = self.lambda_*fixed_q_x_a_base + (1-self.lambda_)*fixed_increace
        ...
        #supply
        supply = self.random_.randint(low=1, high=self.max_supply, size=self.n_actions)
        ...
        for i in range(self.n_step):
            ...
            pi_b_logits_ = pi_b_logits[user_idx]
            pi_b_logits_ = self.beta * pi_b_logits_
            pi_b_logits_[supply<=0] = -10000
            ...
            pscore_list.append(pi_b[action])
