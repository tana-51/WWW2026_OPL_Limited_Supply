...
np.random.seed(0)
num_runs = 100
n_users_array = np.array([5, 10,50,100,200,400,600,800,1000])
result_list = []
for n_users in n_users_array:
    n_actions = 100
    reward_std = 0.1
    lambda_ = 0.0
    n_step = 1000
    ...
    for _ in tqdm(range(num_runs), desc=f"n_users = {n_users}"):
    ...
    plt.plot((new/num_runs)/(previous/num_runs), label=f"$\lambda$={lambda_}, ratio={n_users/n_actions}")
...
# plt.savefig("output_opl/val_lambda.png")
plt.show()

#user-action-ratio vs -relative reward
last_value = []
for i, j in enumerate(n_users_array):
