...
class PreviousAgent():
    ...
    def select_arm(self, user_idx, fixed_q_x_a, supply):
        ...
        regret_value = self.q_max[np.argmax(user_q)] - fixed_q_x_a[user_idx,np.argmax(user_q)]
        if (supply>=1).sum()==0:
            regret_value = 0
        ...
        return np.argmax(user_q), regret_value
...
class NewAgent():
    ...
    def select_arm(self, user_idx, fixed_q_x_a, supply):
        ...
        if ((supply>=1).sum()!=0) and (supply[selected_action] < 1):
            raise ValueError(f"supply must be above 1, but got supply={supply[selected_action]} about action {selected_action}")
        return selected_action, regret_value
